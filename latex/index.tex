...
$>$Note\+: if you are able to run the programs or have those packages installed in a different software or environment. You can skip the Setup section or (You could install the packages with Pip.)\hypertarget{index_autotoc_md3}{}\doxysubsection{Setup}\label{index_autotoc_md3}
...
Recall that num\+Py, pandas, matplotlibpyplot, and stats packages are already installed in Jupyter notebook, you only need to install Tensor\+Flow. Using the Anaconda Prompt, it can be done by entering\+:
...
{\itshape Please make sure they all are in the same directory so the scripts would be able to recognize the data sets from the data folder.}
